<div x-show="isModalOpen" x-transition class="fixed inset-0 z-[80] overflow-y-auto bg-gray-900 bg-opacity-50" style="display: none;">
  <div class="relative w-full max-w-lg p-4 mx-auto mt-7 bg-white border shadow-sm rounded-xl">
      ...
      <div class="flex justify-between items-center py-3 px-4 border-b">
        ...
        <button type="button" @click="closeModal" class="size-7 inline-flex justify-center items-center gap-x-2 rounded-full border border-transparent bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-none focus:bg-gray-200 disabled:opacity-50 disabled:pointer-events-none">
          <span class="sr-only">Close</span>
          <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
        </button>
      </div>
      <form id="taskForm" @submit.prevent="submitTask" enctype="multipart/form-data">
        ...
        <div class="flex justify-end items-center gap-x-2 py-3 px-4 border-t">
          <button type="button" @click="closeModal" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50">Close</button>
