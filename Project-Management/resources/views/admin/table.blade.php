        ...
        @forelse($tasks as $task)
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                    @can('manage-tasks')
                        <button type="button" onclick="openEditModal({{ $task->id }})" class="inline-flex items-center text-blue-600 hover:text-blue-700 font-medium" aria-label="Edit" title="Edit">
                    ...
                    @endcan
                    @can('delete-task')
                        <button type="button" onclick="deleteTask({{ $task->id }})" class="inline-flex items-center text-red-600 hover:text-red-700 font-medium ms-3" aria-label="Delete" title="Delete">
                            <svg class="size-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
